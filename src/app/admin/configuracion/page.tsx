...
import { Save, Plus, Trash2, Calendar, MapPin, Gift, Clock, ShieldCheck, Shirt, ArrowUp, ArrowDown, Eye, X } from 'lucide-react';
...
export default function AdminConfiguracion() {
  ...
  return (
    <div className="h-full bg-black text-white p-4 md:p-10 overflow-y-auto selection:bg-amber-500 selection:text-black">
      <div className="max-w-4xl mx-auto pb-24 space-y-10">
        ...
        <div className="flex items-center justify-between border-b border-white/5 pb-6">
          <div>
            ...
            <p className="text-neutral-500 text-xs mt-1 font-mono">Gestión dinámica de itinerario, dresscode y datos bancarios.</p>
          </div>
        ...
        {/* 2. DRESS CODE Y PALETA DE COLORES */}
        <section className="bg-neutral-950 p-6 rounded-2xl border border-white/10 space-y-4">
          <div className="flex items-center justify-between border-b border-white/5 pb-3">
          ...
          {/* PALETA DE CÍRCULOS DE VESTIMENTA */}
          <div className="space-y-3 pt-2">
            <div className="flex justify-between items-center">
              <label className="text-xs font-mono text-neutral-400">Muestras de Color Permitidas:</label>
              <button onClick={() => setDressCode({ ...dressCode, colors: [...dressCode.colors, '#171717'] })} className="px-3 py-1 bg-neutral-900 border border-white/10 rounded-lg text-xs flex items-center gap-1 text-white">
                <Plus size={12} /> Color
              </button>
            </div>
            <div className="flex flex-wrap gap-3">
              {dressCode.colors?.map((colorHex, idx) => (
                <div key={idx} className="flex items-center gap-2 bg-black/60 p-2 rounded-xl border border-white/10">
                  <input type="color" value={colorHex} onChange={(e) => { const colors = [...dressCode.colors]; colors[idx] = e.target.value; setDressCode({ ...dressCode, colors }); }} className="w-8 h-8 rounded cursor-pointer bg-transparent" />
                  <span className="text-xs font-mono text-neutral-400">{colorHex}</span>
                  <button onClick={() => { const colors = dressCode.colors.filter((_, i) => i !== idx); setDressCode({ ...dressCode, colors }); }} className="text-neutral-500 hover:text-red-500 p-1"><Trash2 size={14} /></button>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* 3. ITINERARIO INTERACTIVO CARDS */}
        <section className="bg-neutral-950 p-6 rounded-2xl border border-white/10 space-y-4">
          <div className="flex items-center justify-between border-b border-white/5 pb-3">
            <h3 className="font-serif text-lg text-amber-400 flex items-center gap-2"><Clock size={18} /> Itinerario (Tarjetas Reordenables)</h3>
            <div className="flex gap-2">
            ...
            {itinerary.map((item, i) => (
              <div key={i} className="flex flex-col md:flex-row gap-3 p-4 bg-black/60 rounded-2xl border border-white/5 items-center shadow-lg">
                <div className="flex md:flex-col gap-1 shrink-0">
                  <button onClick={() => moveItineraryItem(i, 'up')} disabled={i === 0} className="p-1.5 bg-neutral-900 hover:bg-neutral-800 disabled:opacity-30 rounded text-neutral-300"><ArrowUp size={14} /></button>
                  <button onClick={() => moveItineraryItem(i, 'down')} disabled={i === itinerary.length - 1} className="p-1.5 bg-neutral-900 hover:bg-neutral-800 disabled:opacity-30 rounded text-neutral-300"><ArrowDown size={14} /></button>
                </div>
      ...
      {/* MODAL DE PREVISUALIZACIÓN EN TIEMPO REAL */}
      {activePreviewModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[999] flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-neutral-950 border border-white/10 rounded-3xl p-8 max-w-2xl w-full relative shadow-2xl space-y-6">
            <div className="flex justify-between items-center border-b border-white/10 pb-4">
              <h3 className="text-xl font-serif text-amber-400">Previsualización Live</h3>
            ...
            <div className="p-6 bg-black rounded-2xl border border-white/5">
              ...
              {activePreviewModal === 'dress' && (
                <div className="text-center space-y-6">
                  <h3 className="text-3xl font-serif text-white">{dressCode.title}</h3>
                  <div className="flex justify-center gap-4">
                    {dressCode.colors?.map((c, i) => (
                      <div key={i} className="w-12 h-12 rounded-full border border-white/20" style={{ backgroundColor: c }} />
                    ))}
                  </div>
                  <p className="text-xs text-red-400 font-mono uppercase bg-red-950/40 p-3 rounded-xl border border-red-500/20">{dressCode.note}</p>
                </div>
              )}
              ...
              {activePreviewModal === 'itinerary' && (
                <div className="space-y-4">
                  {itinerary.map((item, i) => (
                    <div key={i} className="bg-neutral-900/60 p-4 rounded-xl border border-white/5 flex justify-between items-center">
                      <div>
                        <span className="text-amber-500 font-mono text-xs">{item.time}</span>
                        <h5 className="font-serif font-bold text-white text-base">{item.title}</h5>
