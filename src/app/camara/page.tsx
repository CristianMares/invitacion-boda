...
import { Camera, SwitchCamera, Zap, ImagePlus, Paintbrush, AlertTriangle, Video, Smartphone } from 'lucide-react';
  ...
  const uploadToBackend = async (dataUri: string, source: string) => {
    ...
    try {
      const res = await fetch('/api/upload', {
        ...
        body: JSON.stringify({ image: dataUri, source, filter: 'normal' }), // Nativo no lleva filtro web
      });
    ...
    return (
      <div className="min-h-screen bg-[#111] flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-md space-y-6">
          ...
          <button onClick={() => nativeCamRef.current?.click()} className="w-full bg-[#1a1a1a] hover:bg-[#222] border border-neutral-800 p-6 rounded-2xl flex items-center gap-4 transition-all active:scale-95">
            <div className="bg-emerald-500/10 p-4 rounded-full text-emerald-500"><Smartphone size={32} /></div>
            <div className="text-left">
              <h3 className="text-white font-bold">Cámara Nativa / Video</h3>
              <p className="text-neutral-500 text-sm">Usa el zoom y app original de tu celular.</p>
            </div>
          </button>
          ...
          <button onClick={() => fileInputRef.current?.click()} className="w-full bg-[#1a1a1a] hover:bg-[#222] border border-neutral-800 p-6 rounded-2xl flex items-center gap-4 transition-all active:scale-95">
            ...
            <div className="text-left">
              ...
              <p className="text-neutral-500 text-sm">Sube fotos o videos ya guardados.</p>
            </div>
          </button>

          {/* Inputs ocultos */}
          <input type="file" accept="image/*,video/*" ref={fileInputRef} className="hidden" onChange={(e) => processFile(e, 'upload')} />
          <input type="file" accept="image/*,video/*" capture="environment" ref={nativeCamRef} className="hidden" onChange={(e) => processFile(e, 'native_camera')} />
...
// COMPONENTE SECUNDARIO: La interfaz de cámara optimizada
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col overflow-hidden">
      ...
      <div className="flex-1 relative bg-neutral-900 overflow-hidden">
        ...
        <button onClick={toggleFlash} className={`absolute top-6 right-6 z-50 p-3 rounded-full backdrop-blur-md border transition-all ${flashOn ? 'bg-amber-500 border-amber-400 text-white' : 'bg-black/40 border-white/20 text-white'}`}>
          <Zap size={20} />
        </button>

        <button onClick={goBack} className="absolute top-6 left-6 z-50 px-4 py-2 rounded-full backdrop-blur-md bg-black/40 border border-white/20 text-white text-xs uppercase tracking-widest">
          Volver
        </button>

        <div className="absolute inset-x-0 bottom-6 flex justify-center z-50 pointer-events-none">
          <span className="bg-black/60 backdrop-blur px-3 py-1 rounded text-amber-400 text-xs uppercase tracking-widest border border-white/10">
        ...
        <Webcam
          ...
          className={`absolute inset-0 w-full h-full object-cover transition-all duration-300 ${FILTERS[filterIdx].css}`}
        />
      </div>

      {/* CHASIS (20% inferior) - Plástico Rugoso */}
      <div className="h-40 bg-[#151515] border-t-8 border-[#0a0a0a] shadow-[inset_0_2px_4px_rgba(255,255,255,0.05)] flex items-center justify-around px-4 relative z-40">
        ...
        {/* Filtro (Izquierda) */}
        <button onClick={toggleFilter} className="w-14 h-14 bg-gradient-to-br from-[#2a2a2a] to-[#111] rounded-full flex items-center justify-center shadow-[0_4px_10px_rgba(0,0,0,0.8),inset_0_1px_1px_rgba(255,255,255,0.1)] border border-[#333] text-neutral-400 hover:text-white transition-all active:scale-95 z-10">
          <Paintbrush size={20} />
        </button>
        ...
        <div className="relative z-10">
          ...
          <button 
            ...
            className="relative w-20 h-20 bg-gradient-to-b from-[#e5e5e5] to-[#a3a3a3] rounded-full flex items-center justify-center shadow-[0_8px_15px_rgba(0,0,0,0.8),inset_0_4px_4px_rgba(255,255,255,0.8)] active:shadow-[0_2px_5px_rgba(0,0,0,0.5),inset_0_2px_6px_rgba(0,0,0,0.4)] active:translate-y-[2px] transition-all disabled:opacity-50"
          >
            {isUploading ? (
        ...
        <button onClick={toggleCamera} className="w-14 h-14 bg-gradient-to-br from-[#2a2a2a] to-[#111] rounded-full flex items-center justify-center shadow-[0_4px_10px_rgba(0,0,0,0.8),inset_0_1px_1px_rgba(255,255,255,0.1)] border border-[#333] text-neutral-400 hover:text-white transition-all active:scale-95 z-10">
